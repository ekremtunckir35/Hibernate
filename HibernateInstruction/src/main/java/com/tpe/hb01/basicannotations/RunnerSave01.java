...
public class RunnerSave01 {
    public static void main(String[] args) {
        ...
        student3.setId(1003);
        student3.setName("Sharlock Holmes");
        ...
        Transaction transaction =session.beginTransaction();//transaction(db de atomik işlem birimi) başlatıldı.

        //student1 i tabloya ekleyelim INSERT INTO T_student VALUES(...)

        session.save(student1);
        session.save(student2);
        session.save(student3);

        transaction.commit();//transaction onaylar ve sonlandirir
        session.close();
        sessionFactory.close(); //uygulama tamamlandigi zaman sessionFactory kapatilir.
